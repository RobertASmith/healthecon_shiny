...
The Sick-Sicker model is a 4 state Markov model in which the population can be either Healthy, Sick, Sicker or Dead. The cohort progresses through the model in cycles of equal duration, with the proportion of those in each health state in the next cycle being dependant on the proportion in each health state in the current cycle *m_TR[t, ]* and the transition probability matrix *m_P*.       
The analysis incorporates PSA by creating a data-frame of PSA inputs (one row being one set of PSA inputs) based on cost, utility and probability distributions using the function *f_gen_psa*. The model is then run for each set of PSA inputs one at a time using the model function *f_MM_sicksicker*. Note that we adapt the coding framework from Alarid-Escudero et al., (2019) to use the *f_* prefix for functions.
