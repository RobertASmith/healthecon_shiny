...
The method so far has involved wrapping the model into a function, which takes some inputs and returns a data-frame of outputs, the next steps involve integrating the model function into a shiny web-app. This is all done within a R file which we call *app.R* (https://github.com/RobertASmith/healthecon_shiny/tree/master/App). In order for this to work please ensure that the shiny function is installed, using *install.packages("shiny")* if it is not.

### Set-up

The set-up is relatively simple, load the R-Shiny package from your library so that you can use the R-Shiny function. The next step is to use the *source* function in baseR to load the wrapper function, being careful to ensure your relative path is correct ('./wrapper.R' should work if the app.R file is within the same folder).
