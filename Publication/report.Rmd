...
While, from a transparency perspective, it is preferable that models constructed in R are made open-access to improve replicability and collaboration, it is not a requirement. Sensitive and proprietary data and/or models can be shared internally. or through secure, password-protected web applications, negating the need to email zipped folders. 
...
In this paper, we demonstrated how to generate a user-friendly interface for an economic model programed in R, using the shiny package. This tutorial shows that the process is relatively quick and requires limited additional programming knowledge than that required to build a decision model in R.
