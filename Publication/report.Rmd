...
Health economic models have typically been built in spreadsheet software, but high level programming languages are increasingly being used as model complexity and computational requirements increase. Of all the programming languages there is a particular push towards the use of R  because it is commonly used by statisticians, has a plethora of user created packages and is highly flexible, transparent and adaptable (Incerti et al., 2019; Jalal et al., 2017). However, as a script based approach R's weakness is that it does not have a user interface. The CRAN package 'shiny', which allows programmers to create web-browser user interfaces which interact with R functions, resolves this limitation. This tutorial uses a 4 state markov model used as a teaching tool by the DARTH group as a case-study to demonstrate a simple method of creating an R-Shiny application from an existing R model (Alarid-Escudero et al., 2020). A more detailed tutorial, all code and data are provided on a GitHub repository here: https://robertasmith.github.io/healthecon_shiny/.


# Introduction
  
As the complexity and computational requirements of decision models increase there is growing recognition of the advantages of high level programming languages which support statistical analysis (e.g. R, Python, C++, MATLAB, Java). Of these R, an open-source functional programming language, appears to be ahead of the competition for decision modelling (Jalal et al. 2017). R is free, supported by a large community of statisticians and health economists and has extensive resources including tutorials, guidelines and packages which allow users to quickly ustilise methods and code created by others. Importantly for the UK is also currently the only programming environment accepted by NICE for HTA submissions, the alternative submission formats Excel, DATA/Treeage, and WinBUGs are all software applications (NICE, 2014). 

Despite the many strengths of the script based approach to decision modelling (e.g R), an important limitation until recently has been the lack of a easily created user interface. Even spreadsheet software, with its many limitations, allows the user to create a front tab in which a decision maker can change a parameter in a specific cell. Incerti et al. (2019) identify web applications as being an essential part of any model in R, stating that they "believe that the future of cost-effectiveness modeling lies in web apps, in which graphical interfaces are used to run script-based models" (p. 577).  

R-Shiny is a package which programmers can use to create a graphical interface which looks like a website, and allows users to interact with underlying R models (Beely, 2013). While there are alternatives, such as Dash for Python, Baio & Heath (2017) identify "web applications created using R Shiny"(p.e5) to be the "future of applied statistical modelling, particularly for cost-effectiveness analysis". Using Shiny, it is possible to create flexible user interfaces which allow users to change parameters, run underlying R code and display results. The primary benefit of this is that it makes script based models that are technically transparent to those with programming knowledge open for sensitivity analysis to those with no programming knowledge (Jansen et al., 2019). Other benefits involve leveraging the power of other R packages, for example allowing for publication quality graphs and tables to be downloaded, user specific data uploaded, open-access data automatically updated and perhaps best of all, restricted password protected access online negating the need to email zipped folders.

The benefits of utilising this method are greater for decision models which require updating, which may be used by different decision makers to answer variations of the same question, and which cover different geographical regions, organisations or perspectives. In short it allows for timely and specific sensitivity analysis which would be overwhelming if included in a report. It is particularly useful in cases where the model inputs may be unknown, or quickly outdated, such that constantly recreating the model would be unfeasible and it is far more efficient to allow the user to input updated parameters, and assuming the same structure, run the updated model.
...
While the aim of this tutorial is not to describe the DARTH model, readers are encouraged to read the cited texts above, it is important to have a basic understanding. The sick-sicker model contains two functions, the first, *f_gen_psa* creates a set of psa inputs, the second, *f_MM_sicksicker* runs the model for a specific set of PSA inputs.
