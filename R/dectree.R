...
f_model = function(n_draws = 1000,      # number of PSA iterations
  ...
  df_res = data.frame(c_incr,u_incr,icer)
  return(df_res)
}


df_out = f_model(n_draws = 1000,
                 rr = 0.95,s = 0.01,
                 c_B_tr_mean = 1000,c_B_tr_sd = 200,
                 p_A_S1 = 0.05)

plot(x=df_out$u_incr,y=df_out$c_incr,
     cex=0.8,col="cadetblue4",
     xlab = "incremental life years",
     ylab = "incremental costs",
     xlim = range(c(0,df_out$u_incr)),
     ylim = range(c(0,df_out$c_incr))
)
abline(h=0,v=0)
